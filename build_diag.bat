...
:: -DPSR_TRACE_HOOKS=OFF so the recompiled funcs record the trace_recent ring
:: (guest function-entry history) for crash RE (issues #9/#16 gallery null-vtable).
:: Logs to _cfg_diag.log (configure) and _build_diag.log (build).
setlocal
...
echo === CONFIGURE (PSR_TRACE_HOOKS=OFF) ===
"%CMAKE%" -S "%PROJ%" -B "%PROJ%\build" -G Ninja -DCMAKE_C_COMPILER=clang-cl -DCMAKE_CXX_COMPILER=clang-cl -DCMAKE_BUILD_TYPE=Release -DPSR_NO_CONSOLE=ON -DWITH_ARES_BRIDGE=OFF -DPSR_TRACE_HOOKS=OFF -DPSR_GEO_PROBE=ON > "%PROJ%\_cfg_diag.log" 2>&1
...
"%CMAKE%" --build "%PROJ%\build" --target PokemonStadiumRecomp -- -j 6 > "%PROJ%\_build_diag.log" 2>&1
